...
{
	i2cdev.writeByte(ADXL345_ADDRESS, reg, value);
}
...
{
	uint8_t value;
	i2cdev.readByte(ADXL345_ADDRESS, reg, &value);
...
{
	uint16_t value;
	i2cdev.readWord(ADXL345_ADDRESS, reg, &value);
...
{
	uint8_t value;
	i2cdev.writeBit(ADXL345_ADDRESS, reg, pos, state);
}

bool ADXL345::readRegisterBit(uint8_t reg, uint8_t pos)
{
	uint8_t value;
	i2cdev.readBit(ADXL345_ADDRESS, reg, pos, &value);
