...
{
	i2cdev.writeByte(addr, QMC5883L_CONFIG, oversampling | range | rate | mode);
}

void QMC5883L::reset()
{
	i2cdev.writeByte(addr, QMC5883L_RESET, 0x01);
	reconfig();
}
...
{
	uint8_t status =0;
	i2cdev.readByte(addr, QMC5883L_STATUS, &status);
...
int QMC5883L::readRaw(int16_t *x, int16_t *y, int16_t *z, int16_t *t)
{
	...
	int16_t buffer[3];
	i2cdev.readWords(addr, QMC5883L_X_LSB, 3, (uint16_t*)buffer);
	*x = buffer[0];
	*y = buffer[1];
	*z = buffer[2];
